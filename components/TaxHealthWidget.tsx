...
export function TaxHealthWidget() {
    ...
    if (loading) {
        return (
            <Card className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden relative">
    ...
    return (
        <Card className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-foreground shadow-xl overflow-hidden relative">
            {/* Background Decor */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-amber-500/5 dark:bg-amber-500/10 rounded-full blur-3xl -mr-32 -mt-32 pointer-events-none"></div>
            ...
            <CardHeader className="pb-2 relative z-10">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <CardDescription className="text-muted-foreground">Salud impositiva del mes</CardDescription>
                    </div>
            ...
            <CardContent className="relative z-10">
                <div className="flex flex-col md:flex-row gap-6 items-end">
                    ...
                    <div className="flex-1">
                        <p className="text-muted-foreground text-xs uppercase tracking-wider font-semibold mb-1">Reserva Sugerida (DGII)</p>
                        ...
                        <div className="flex items-center gap-2 text-xs">
                            ...
                            {taxData.netTaxPayable > 0 ? (
                                <span className="text-amber-700 dark:text-amber-400 flex items-center bg-amber-500/10 px-2 py-0.5 rounded">
                                    <AlertTriangle className="w-3 h-3 mr-1" />
                                    Apartar dinero
                                </span>
                            ) : (
                                <span className="text-emerald-700 dark:text-emerald-400 flex items-center bg-emerald-500/10 px-2 py-0.5 rounded">
                    ...
                    {/* Breakdown */}
                    <div className="flex-1 space-y-3 bg-slate-50 dark:bg-white/5 p-4 rounded-xl border border-slate-100 dark:border-white/10">
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-slate-600 dark:text-slate-300 flex items-center">
                                <ArrowUp className="w-3 h-3 text-emerald-500 dark:text-emerald-400 mr-2" />
                                ITBIS Cobrado
                            </span>
                        ...
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-slate-600 dark:text-slate-300 flex items-center">
                                <ArrowDown className="w-3 h-3 text-amber-600 dark:text-amber-500 mr-2" />
                                ITBIS Deducible
                            </span>
                            <span className="font-semibold text-rose-600 dark:text-red-400">-{formatCurrency(taxData.paidItbis)}</span>
                        </div>
